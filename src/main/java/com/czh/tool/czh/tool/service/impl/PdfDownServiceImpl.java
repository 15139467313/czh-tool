...
@Service
@Slf4j
@FileDownType(FileStrategy.PDF)
public class PdfDownServiceImpl implements FileDownService {
    ...
    public void down(Object... args) throws IOException {
       try {
           ...
           FileInputStream fis = new FileInputStream(fileDownConfig.getFileDownPath()+"/"+fileDown.fileName());
            ...
            if (!fileDown.checkSee()){
                response.setHeader("content-disposition", "attachment; filename*=UTF-8''" + URLEncoder.encode(fileDown.fileName(), "UTF-8"));
            }
            int len = 0;
